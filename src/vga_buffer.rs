...
// to prevent writes to VGA buffer from being optimized away
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
// ensure that the ColorCode has the exact same data layout as a u8
#[repr(transparent)]
struct ColorCode(u8);
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
// same memory layout as a C struct
#[repr(C)]
struct ScreenChar {
    ascii_character: u8,
...
// same memory layout as the single 
#[repr(transparent)]
struct Buffer {
...
// implement the core::fmt::Write trait to support formatting macros
